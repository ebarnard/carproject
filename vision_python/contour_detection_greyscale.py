...
while True:
    ...
    cv2.floodFill(thresh, mask, (bbox[0], bbox[1]), 255, flags=4 | 255 << 8 | cv2.FLOODFILL_MASK_ONLY)
    ...
    if i == 0:
        image_average = imgray * (mask[1:-1, 1:-1] == 0)
        i += 1

    cv2.addWeighted(image_average, 0.95, imgray * (mask[1:-1, 1:-1] == 0), 0.05, 0, image_average)

    ret, track = cv2.threshold(image_average, 1, 255, 0)

    image_use = imgray.copy()
    cv2.addWeighted(image_use, 1, image_average, -1, 0, image_use)
    image_use = image_use * (track != 0)

    # show the image
    cv2.imshow("Image", image_use)
    cv2.waitKey(0)

    image_use = thresh * (image_average > 5) * (mask[1:-1, 1:-1] != 0)

    # show the image
    cv2.imshow("Image", image_use)
    cv2.waitKey(0)

    im2, contours, hierarchy = cv2.findContours(image_use, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)
